 ...
 * If dist/server.cjs is missing, it will automatically build the project using Node directly
 * (without needing global npm in the system PATH) and start the server seamlessly.
 */
...
/**
 * Executes a command using process.execPath (Node) or fallback child processes
 */
function runCommand(command, args, cwd) {
  return new Promise((resolve, reject) => {
    const nodeBinDir = path.dirname(process.execPath);
    const localBinDir = path.join(cwd, 'node_modules', '.bin');
    const pathEnv = `${localBinDir}:${nodeBinDir}:${process.env.PATH || ''}`;
    ...
    child.stdout.on('data', (data) => {
      addBuildLog(data.toString().trim());
    });
...
async function performBuild() {
  ...
  addBuildLog("2/2 Compilando backend com esbuild...");
  try {
          ...
          <head>
            ...
            <style>
              * { box-sizing: border-box; margin: 0; padding: 0; }
              ...
              .terminal { background: #090d16; border: 1px solid #1e293b; border-radius: 8px; padding: 16px; font-family: monospace; font-size: 13px; color: #a5f3fc; max-height: 250px; overflow-y: auto; white-space: pre-wrap; word-break: break-all; margin-bottom: 24px; }
              ...
              .instructions { margin-top: 24px; padding-top: 20px; border-top: 1px solid #334155; font-size: 13px; color: #94a3b8; line-height: 1.6; }
              .instructions code { background: #0f172a; padding: 2px 6px; border-radius: 4px; color: #38bdf8; font-weight: bold; }
            </style>
          </head>
          <body>
            <div class="card">
              ...
              <p>O arquivo compilado <code>dist/server.cjs</code> está sendo gerado automaticamente.</p>
              
              <div id="terminal" class="terminal">${buildLogs.join('\n') || error?.message || 'Aguardando compilação...'}</div>
              
              <div style="display: flex; gap: 12px;">
                <button id="btnBuild" class="btn" onclick="triggerBuild()">⚡ Executar Compilação Agora</button>
                <button class="btn" style="background: #334155;" onclick="location.reload()">🔄 Atualizar Página</button>
              </div>

              <div class="instructions">
                <strong>Configuração recomendada na Hostinger (Painel hPanel):</strong><br>
                No menu <strong>Implantações</strong> &gt; <strong>Configurações de compilação</strong>:<br>
                • Comando de compilação: <code>npm run build</code><br>
                • Comando de inicialização: <code>node dist/server.cjs</code>
              </div>
            </div>
