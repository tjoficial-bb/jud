...
export const AssessorPitchAndTipsCard: React.FC<AssessorPitchAndTipsCardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'investidor' | 'moradia' | 'dicas' | 'objecoes'>('investidor');
  ...
  const getContextualSummary = (): string => {
    ...
    switch (contextType) {
      case 'matricula':
        return `A auditoria registral confirma a higidez da cadeia de domínio da matrícula. Todos os gravames anteriores (penhoras, hipotecas e alienações) serão cancelados e baixados com a expedição da Carta de Arrematação pelo juiz ou consolidação extrajudicial. A aquisição proporciona titularidade 100% livre e desembaraçada no Cartório de Registro de Imóveis competente.`;
      case 'edital':
        return `O edital apresenta condições comerciais excepcionais, com desconto de até ${discountPercent}% na 2ª praça. Há previsão expressa de sub-rogação de débitos fiscais (IPTU) no preço arrematado (Art. 130 CTN) e possibilidade de arrematação parcelada (25% de entrada + saldo em até 30 parcelas pelo Art. 895 CPC), alavancando expressivamente o retorno.`;
      case 'processo':
        return `O processo judicial foi instruído com citação e intimações regulares de todas as partes e coproprietários. O risco de nulidade é controlado e a arrematação confere ao comprador o direito imediato ao Mandado de Imissão na Posse expedido diretamente pelo Magistrado, garantindo segurança jurídica irrestrita.`;
      case 'smart':
        return `A Análise Smart de Riscos consolidada classifica esta oportunidade com excelente índice de viabilidade. Os riscos operacionais (desocupação e custos cartorários) estão devidamente mapeados e precificados na simulação, assegurando alta margem de segurança e proteção de capital.`;
      case 'assessoria':
        return `Parecer de assessoria jurídica favorável: o imóvel possui documentação apta para arrematação com ampla segurança patrimonial. A estratégia de arrematação recomendada maximiza o desconto e prevê assistência completa desde o lance até a entrega das chaves.`;
      case 'dossier':
      case 'master':
      default:
        return `Oportunidade de leilão selecionada e auditada com excelente viabilidade técnico-jurídica. Avaliado em ${formatMoney(valuation)} com lance inicial em ${formatMoney(minBid)}, proporcionando uma margem bruta atrativa tanto para investidores focados em revenda/renda quanto para famílias que buscam moradia de alto padrão pagando metade do valor.`;
    }
  };
  ...
  const generateInvestorPitch = (): string => {
    ...
    return `*🎯 OPORTUNIDADE EXCLUSIVA DE INVESTIMENTO EM LEILÃO*\n\n` +
      ...
      `Já realizamos a auditoria completa de edital, matrícula e processo. O imóvel conta com viabilidade jurídica aprovada e possibilidade de pagamento parcelado.\n\n` +
  ...
  const handleCopy = async (text: string, setCopied: (v: boolean) => void) => {
    try {
      ...
      if ((window as any).customToast) {
        (window as any).customToast("Pitch copiado com sucesso! Cole diretamente no WhatsApp.", "success");
      }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h4 className="text-base sm:text-lg font-black text-brand-ink tracking-tight flex items-center gap-2 mt-0.5">
              Resumo Executivo & Dicas de Captação Comercial
            </h4>
          </div>
        </div>
        ...
        <div className="pt-4 space-y-5">
          ...
          {(valuation > 0 || minBid > 0) && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              ...
              <div className="bg-brand-paper border border-brand-border rounded-xl p-3 flex flex-col">
                <span className="text-[10px] font-bold text-brand-ink/50 uppercase tracking-wider">Lance de Oportunidade</span>
              ...
              <div className="bg-brand-paper border border-brand-border rounded-xl p-3 flex flex-col">
                <span className="text-[10px] font-bold text-brand-ink/50 uppercase tracking-wider">Economia / Margem</span>
          ...
          <div className="flex flex-wrap items-center gap-1.5 p-1 bg-brand-bg/60 rounded-2xl border border-brand-border/60">
            ...
            >
              <Home size={15} />
              <span>Pitch para Moradia / Família</span>
            </button>
            ...
            >
              <Lightbulb size={15} />
              <span>Dicas de Ouro do Assessor</span>
            </button>
            ...
            <div className="bg-brand-paper border border-brand-border rounded-2xl p-4 sm:p-5 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-brand-border/50">
                ...
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-brand-primary text-white hover:bg-brand-primary/90 text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition-transform active:scale-95 shrink-0"
                >
              ...
              <div className="flex items-center gap-2 text-[11px] text-brand-ink/60 pt-1">
                ...
                <span>💡 Dica de abordagem: Envie este texto acompanhado de 1 foto ou PDF resumido do imóvel para aumentar a taxa de resposta.</span>
              </div>
            </div>
          )}
          ...
          {activeTab === 'moradia' && (
            <div className="bg-brand-paper border border-brand-border rounded-2xl p-4 sm:p-5 space-y-3">
              ...
              <div className="flex items-center gap-2 text-[11px] text-brand-ink/60 pt-1">
                ...
                <span>🏡 Dica de abordagem: Mostre ao cliente que o dinheiro economizado no leilão pode pagar toda a reforma e decoração de luxo do imóvel.</span>
              </div>
            </div>
          )}

          {/* Tab 3: Dicas de Ouro do Assessor */}
          {activeTab === 'dicas' && (
            <div className="bg-brand-paper border border-brand-border rounded-2xl p-4 sm:p-5 space-y-3">
